...
function encodeV2(state, registry) {
  ...
  if (state.standingStoneId) payload.s = registry.standingStoneIndex.get(state.standingStoneId);
  if (state.blessingId) payload.b = registry.blessingIndex.get(state.blessingId);
...
const standingStones = loadJson("data/game/standing-stones.json").standingStones.map((s) => s.id);
const blessings = loadJson("data/game/blessings.json").blessings.map((b) => b.id);
...
const samples = [
  ...
  },
  {
    ...
    state: {
      raceId: "nord",
      standingStoneId: "lover",
      blessingId: "blessing-of-akatosh",
      traitIds: ["athletic"],
  ...
  },
  {
    ...
    state: {
      raceId: "orc",
      standingStoneId: "warrior",
      blessingId: "blessing-of-zenithar",
